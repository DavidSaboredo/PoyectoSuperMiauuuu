...
// Configuración interna del motor de físicas
...
// Cada mundo conserva el mismo control, pero los mapas que exigen
// precisión tienen una velocidad máxima un poco menor. La aceleración
// progresiva evita que Miau pase de quieto a máxima velocidad de golpe.
const LAND_MOVEMENT = {
...
// Nivel 1.3: los semáforos ahora gobiernan realmente el tránsito.
// Durante la fase roja los autos frenan antes de la senda peatonal.
...
function initLevel() {
    ...
    if (currentLevel === 2 && levelTwoSection === 3) {
        ...
        // El aerosol se descubre después de las primeras bandadas, de
        // modo que el jugador aprende el peligro antes de encontrar
        // la herramienta que permite espantarlas.
        batSprayPickup = {
            x: 2860,
            y: 337,
            width: 24,
    ...
    if (currentLevel !== 6 && !vampireExitLocked && !flagpole.reached && checkCollision(player, flagpole)) {
        ...
        if (currentLevel === 1) {
            ...
            if (levelTwoSection === 1) {
                ...
                showOverlay('🦇', '¡COFRES ENTRE LAS RAÍCES!', 'El sendero zombi termina frente a una zona más oscura. Hay cofres antiguos por todo el camino: acercarte puede liberar una bandada de murciélagos.', 'Entrar al nivel 2.3 🦇');
            } else {
...
function drawStreetEnvironmentV21() {
    ...
    const road = ctx.createLinearGradient(0, 312, 0, 378);
    road.addColorStop(0, 'rgba(40,50,60,.58)');
    road.addColorStop(.55, 'rgba(25,34,43,.78)');
    road.addColorStop(1, 'rgba(13,23,31,.94)');
    ctx.fillStyle = road;
    ctx.fillRect(0, 312, canvas.width, 66);
    ctx.fillStyle = 'rgba(245,214,111,.72)';
    for (let x = -((cameraX * .94) % 172); x < canvas.width; x += 172) {
        ctx.fillRect(x, 349, 84, 3);
    }

    // Cordon + vereda mojada. Miau pisa esta franja; los autos quedan
    // visualmente del otro lado del cordon.
    ctx.fillStyle = '#c8c2b4';
    ctx.fillRect(0, 376, canvas.width, 4);
...
function drawStreetEnvironment() {
    ...
    // V21: una unica escena ilustrada que se recorre progresivamente.
    drawStreetEnvironmentV21();
    return;

    // V11: background del 1.3 dibujado íntegramente desde cero.
    // Cielo, ciudad, vereda y avenida forman una única escena; no se
    // renderiza debajo la atmósfera del parque de 1.1/1.2.
    const sky = ctx.createLinearGradient(0, 0, 0, 310);
    sky.addColorStop(0, '#020617');
    sky.addColorStop(.48, '#071629');
    sky.addColorStop(.78, '#0d2238');
    sky.addColorStop(1, '#182b3d');
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Resplandor frío y casi cubierto: 1.3 sigue ocurriendo durante la
    // misma noche de tormenta de todo el capítulo 1.
    const sunsetX = 665 - cameraX * .018;
    const sunsetGlow = ctx.createRadialGradient(sunsetX, 118, 4, sunsetX, 118, 115);
    sunsetGlow.addColorStop(0, 'rgba(191,219,254,.16)');
    sunsetGlow.addColorStop(.25, 'rgba(96,165,250,.07)');
    sunsetGlow.addColorStop(1, 'rgba(30,64,175,0)');
    ctx.fillStyle = sunsetGlow;
    ctx.beginPath(); ctx.arc(sunsetX, 118, 115, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = .14;
    ctx.fillStyle = '#dbeafe';
    ctx.beginPath(); ctx.arc(sunsetX, 118, 25, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = 1;

    // Nubes urbanas propias de esta escena. Son anchas y suaves para no
    // confundirse con los árboles/tormenta del parque.
    const cloudOffset = -((cameraX * .035) % 360);
    ctx.fillStyle = 'rgba(30,41,59,.78)';
    [90, 430, 770].forEach((baseX, index) => {
        const cx = baseX + cloudOffset;
        const cy = 74 + index * 31;
        ctx.beginPath();
        ctx.ellipse(cx, cy, 70, 13, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + 50, cy + 2, 45, 10, 0, 0, Math.PI * 2);
        ctx.fill();
    });

    // Silueta lejana: edificios simples y bajos. El borde inferior queda
    // siempre detrás de las fachadas cercanas y de la vereda.
    const farOffset = -((cameraX * .075) % 104) - 104;
    for (let i = 0; i < 11; i++) {
        const bx = farOffset + i * 104;
        const bw = 62 + (i % 3) * 13;
        const bh = 74 + ((i * 31) % 76);
        const by = 264 - bh;
        ctx.fillStyle = i % 2 ? '#13263b' : '#182c41';
        ctx.fillRect(bx, by, bw, bh);
        if (i % 3 === 0) {
            ctx.fillStyle = '#1d3045';
            ctx.fillRect(bx + bw * .45, by - 18, 4, 18);
        }
        ctx.fillStyle = 'rgba(255,222,151,.28)';
        for (let wy = by + 14; wy < 250; wy += 23) {
            for (let wx = bx + 10; wx < bx + bw - 8; wx += 21) {
                if (((i + wy + wx) % 4) !== 0) ctx.fillRect(wx, wy, 7, 8);
            }
        }
    }

    // Fachadas cercanas: una hilera continua de comercios/casas crea una
    // lectura clara de barrio sin invadir la zona jugable.
    const nearOffset = -((cameraX * .16) % 170) - 170;
    const facadeColors = ['#26384d', '#304057', '#21374e', '#343445'];
    const awningColors = ['#e76f51', '#2a9d8f', '#e9c46a', '#577590'];
    for (let i = 0; i < 7; i++) {
        const bx = nearOffset + i * 170;
        const bw = 154;
        const bh = 68 + (i % 3) * 16;
        const by = 282 - bh;
        ctx.fillStyle = facadeColors[i % facadeColors.length];
        ctx.fillRect(bx, by, bw, bh);
        ctx.fillStyle = 'rgba(15,23,42,.28)';
        ctx.fillRect(bx, by, bw, 6);

        // Ventanas superiores.
        ctx.fillStyle = 'rgba(189,228,247,.38)';
        for (let wx = bx + 13; wx < bx + bw - 18; wx += 42) {
            ctx.fillRect(wx, by + 15, 25, 17);
            ctx.fillStyle = 'rgba(255,230,163,.42)';
            ctx.fillRect(wx + 3, by + 18, 19, 11);
            ctx.fillStyle = 'rgba(189,228,247,.38)';
        }

        // Locales a nivel de vereda.
        const shopTop = 249;
        ctx.fillStyle = '#27394d';
        ctx.fillRect(bx + 10, shopTop, bw - 20, 33);
        ctx.fillStyle = awningColors[i % awningColors.length];
        ctx.fillRect(bx + 7, shopTop - 5, bw - 14, 8);
        ctx.fillStyle = 'rgba(186,230,253,.28)';
        ctx.fillRect(bx + 17, shopTop + 8, 44, 22);
        ctx.fillRect(bx + 72, shopTop + 8, 44, 22);
        ctx.fillStyle = '#1f2937';
        ctx.fillRect(bx + 126, shopTop + 6, 18, 27);
    }

    // Vereda ancha y clara. La junta de baldosas se mueve con el mundo,
    // mientras que el asfalto empieza debajo del cordón.
    const sidewalk = ctx.createLinearGradient(0, 282, 0, 306);
    sidewalk.addColorStop(0, '#9aa7b4');
    sidewalk.addColorStop(1, '#667482');
    ctx.fillStyle = sidewalk;
    ctx.fillRect(0, 282, canvas.width, 24);
    ctx.fillStyle = 'rgba(45,55,72,.34)';
    for (let x = -((cameraX * .9) % 78); x < canvas.width; x += 78) ctx.fillRect(x, 283, 2, 18);
    ctx.fillStyle = '#eef2f5';
    ctx.fillRect(0, 302, canvas.width, 5);
    ctx.fillStyle = '#596674';
    ctx.fillRect(0, 307, canvas.width, 3);

    // Una sola superficie de asfalto. El bloque físico de suelo que se
    // dibuja después continúa exactamente esta gama de colores.
    const road = ctx.createLinearGradient(0, 310, 0, canvas.height);
    road.addColorStop(0, '#303a45');
    road.addColorStop(.48, '#242d37');
    road.addColorStop(1, '#111820');
    ctx.fillStyle = road;
    ctx.fillRect(0, 310, canvas.width, canvas.height - 310);

    // Marcas viales de lectura limpia.
    ctx.fillStyle = '#e9c95f';
    for (let x = -((cameraX * .95) % 168); x < canvas.width; x += 168) ctx.fillRect(x, 350, 88, 4);
    ctx.fillStyle = 'rgba(255,255,255,.055)';
    for (let x = -((cameraX * .7) % 126); x < canvas.width; x += 126) ctx.fillRect(x, 401, 38, 2);

    // Sustituye la escenografía genérica por el panorama inspirado en
    // Juan Perón. El recorte avanza suavemente con la cámara: al
    // comienzo domina el Kiosco El Sureño y, hacia el final, la UTN.
    // Los semáforos, autos y demás mecánicas se dibujan después y
    // continúan siendo elementos interactivos del juego.
    if (JUAN_PERON_STREET_BACKGROUND.complete && JUAN_PERON_STREET_BACKGROUND.naturalWidth > 0) {
        const image = JUAN_PERON_STREET_BACKGROUND;
        ...
        const sourceX = maxSourceX * progress;
        ctx.drawImage(image, sourceX, 0, sourceWidth, sourceHeight, 0, 0, canvas.width, canvas.height);

        // Baño frío sutil: mantiene continuidad con la tormenta de 1.1
        // y 1.2 sin tapar los faroles ni los reflejos del asfalto.
        ctx.fillStyle = 'rgba(2,6,23,.08)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
    }

    STREET_CROSSINGS.forEach(crossing => {
        const worldX = crossing.x;
        const screenX = worldX - cameraX;
        if (screenX < -150 || screenX > canvas.width + 80) return;
        const signal = getStreetSignalState(worldX);
        ctx.fillStyle = 'rgba(248,250,252,.86)';
        for (let stripe = 0; stripe < 7; stripe++) ctx.fillRect(screenX + stripe * 20, 312, 11, 66);

        ctx.save();
        ctx.strokeStyle = '#3f4c59'; ctx.lineWidth = 6;
        ctx.beginPath(); ctx.moveTo(screenX - 18, 302); ctx.lineTo(screenX - 18, 212); ctx.stroke();
        ctx.fillStyle = '#1b2632'; traceRoundedRect(ctx, screenX - 33, 211, 30, 72, 8); ctx.fill();
        ctx.strokeStyle = '#64748b'; ctx.lineWidth = 2; traceRoundedRect(ctx, screenX - 32, 212, 28, 70, 7); ctx.stroke();
        ctx.fillStyle = signal.carsStopped ? '#ef4444' : '#641d24'; ctx.beginPath(); ctx.arc(screenX - 18, 230, 8, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = signal.carsStopped ? '#123c2a' : '#22c55e'; ctx.beginPath(); ctx.arc(screenX - 18, 263, 8, 0, Math.PI * 2); ctx.fill();

        // Cartel peatonal: evita que el color del semáforo sea ambiguo.
        ctx.fillStyle = signal.safeToCross ? 'rgba(20,83,45,.96)' : 'rgba(127,29,29,.96)';
        traceRoundedRect(ctx, screenX - 55, 181, 74, 24, 8); ctx.fill();
        ctx.fillStyle = '#f8fafc';
        ctx.font = 'bold 10px Fredoka';
        ctx.textAlign = 'center';
        ctx.fillText(signal.safeToCross ? 'PASÁ' : 'ESPERÁ', screenX - 18, 197);
        ctx.restore();
    });
}
...
function renderGame() {
    ...
    if (currentLevel === 2 && levelTwoSection === 3) {
        if (batSprayPickup && !batSprayPickup.collected && isOnScreen(batSprayPickup.x, batSprayPickup.width, 100)) {
            ...
            ctx.fillStyle = 'rgba(2,6,23,.84)'; traceRoundedRect(ctx, sprayX - 48, sprayY - 27, 120, 20, 7); ctx.fill();
            ctx.fillStyle = '#a7f3d0'; ctx.font = 'bold 9px Fredoka'; ctx.fillText('AEROSOL ANTI-MURCIÉLAGOS', sprayX + 12, sprayY - 13);
            ctx.restore();
        }
...
function drawCat(ctx, x, y, width, height, direction, isMoving, isJumping, tick, isInvulnerable) {
    ...
    // V6: nunca volver a dibujar silenciosamente el personaje viejo.
    // Si una imagen no esta lista, se muestra un aviso imposible de
    // confundir con Super Miau y se espera al siguiente fotograma.
    ctx.save();
    ...
    const centerX = x + width / 2 - cameraX;
    const feetY = y + height;
    const altitude = Math.max(0, WORLD_HEIGHT - feetY);
    const shadowScale = player.grounded ? 1 : Math.max(.3, 1 - altitude / 150);

    ctx.save();
    const shadow = ctx.createRadialGradient(centerX, feetY, 1, centerX, feetY, width * .68);
    shadow.addColorStop(0, `rgba(2,6,23,${.42 * shadowScale})`);
    shadow.addColorStop(1, 'rgba(2,6,23,0)');
    ctx.fillStyle = shadow;
    ctx.beginPath();
    ctx.ellipse(centerX, feetY + 2, width * .72 * shadowScale, 5 * shadowScale, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
    ...
    const characterScale = Math.min(width / 32, height / 32);
    ctx.scale((direction === -1 ? -1 : 1) * characterScale, characterScale);

    const speedRatio = Math.min(1, Math.abs(player.vx) / Math.max(1, player.speed));
    const runPhase = tick * (.2 + speedRatio * .12);
    const stride = isMoving && player.grounded ? Math.sin(runPhase) * 5.2 : 0;
    const idleBreath = !isMoving && player.grounded ? Math.sin(tick * .055) : 0;
    const bodyBob = player.grounded
        ? (isMoving ? -Math.abs(Math.sin(runPhase)) * 1.15 : idleBreath * .55)
        : 0;
    const upward = isJumping && player.vy < 0;
    const falling = isJumping && player.vy >= 0;
    const squashX = upward ? .94 : falling ? 1.035 : 1;
    const squashY = upward ? 1.075 : falling ? .965 : 1;
    const lean = player.grounded ? player.vx / Math.max(1, player.speed) * .045 : Math.max(-.09, Math.min(.09, player.vx * .018));
    ctx.rotate(lean);
    ...
    if (player.powerup === 'strength') {
        ctx.save();
        ctx.globalAlpha = .34 + Math.sin(tick * .18) * .09;
        ctx.strokeStyle = '#fde047';
        ctx.shadowColor = '#facc15';
        ctx.shadowBlur = getShadowBlurValue(20, 9, 2);
        ctx.lineWidth = 2.2;
        ctx.beginPath();
        ctx.ellipse(0, -2, 23 + Math.sin(tick * .12) * 2, 25 + Math.cos(tick * .12) * 2, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
    }

    // Cola con dos tonos y movimiento independiente.
    const tailWave = isJumping
        ? -5 + Math.sin(tick * .11) * 2
        : (isMoving ? Math.sin(runPhase - .7) * 5.5 : Math.sin(tick * .075) * 3.2);
    const tailGradient = ctx.createLinearGradient(-12, 4, -25, -13);
    tailGradient.addColorStop(0, '#c2410c');
    tailGradient.addColorStop(.55, '#f97316');
    tailGradient.addColorStop(1, '#fdba74');
    ctx.strokeStyle = '#7c2d12';
    ctx.lineWidth = 6.8;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(-11, 4);
    ctx.bezierCurveTo(-21, 6 + tailWave, -25, -5 + tailWave, -19, -13 + tailWave * .45);
    ctx.stroke();
    ctx.strokeStyle = tailGradient;
    ctx.lineWidth = 4.6;
    ctx.stroke();

    // Capa roja: se estira al correr, se eleva al saltar y conserva
    // una onda pequeña cuando Miau está quieto.
    const capeLift = upward ? -8 : falling ? -2 : -speedRatio * 3;
    const capeWave = Math.sin(tick * .17 - speedRatio * 2) * (1.2 + speedRatio * 2.1);
    const capeLength = 20 + speedRatio * 7;
    ctx.save();
    ctx.shadowColor = '#7f1d1d';
    ctx.shadowBlur = getShadowBlurValue(7, 3, 0);
    const capeGradient = ctx.createLinearGradient(-7, -9, -27, 12);
    capeGradient.addColorStop(0, '#fb7185');
    capeGradient.addColorStop(.35, '#ef4444');
    capeGradient.addColorStop(1, '#991b1b');
    ctx.fillStyle = capeGradient;
    ctx.beginPath();
    ctx.moveTo(-6, -8);
    ctx.bezierCurveTo(-12, -10 + capeLift, -20, -7 + capeLift, -capeLength, -2 + capeLift + capeWave);
    ctx.quadraticCurveTo(-22, 4 + capeWave, -17, 9 + capeWave * .45);
    ...
    ctx.beginPath();
    ctx.moveTo(-8,-6); ctx.quadraticCurveTo(-16,-7+capeLift,-capeLength+3,-2+capeLift+capeWave); ctx.stroke();
    ctx.restore();

    // Patas traseras y delanteras. En el aire se repliegan; al correr
    // alternan sin cambiar la posicion logica del personaje.
    const tucked = isJumping ? 4 : 0;
    const backLegX = -6 - (isJumping ? 1 : stride * .65);
    const frontLegX = 7 + (isJumping ? 1 : stride * .65);
    const legY = isJumping ? 9 : 13;
    ctx.fillStyle = '#b94712';
    [[backLegX, legY - tucked], [frontLegX, legY - tucked]].forEach(([legX, footY]) => {
        ctx.beginPath(); ctx.ellipse(legX, footY - 2, 4.4, 6.2, stride * .018, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#f7a44f';
        ctx.beginPath(); ctx.ellipse(legX + 1.4, footY + 2.2, 4.6, 2.6, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = '#b94712';
    });

    // Cuerpo con luz cálida superior y sombra fría inferior.
    ctx.strokeStyle = '#7c2d12';
    ctx.lineWidth = 1.5;
    const bodyGradient = ctx.createRadialGradient(5, -1, 2, -1, 5, 18);
    bodyGradient.addColorStop(0, '#ffd08a');
    bodyGradient.addColorStop(.28, '#fb923c');
    bodyGradient.addColorStop(.72, '#ea580c');
    bodyGradient.addColorStop(1, '#9a3412');
    ctx.fillStyle = bodyGradient;
    ctx.beginPath(); ctx.ellipse(-1, 3, 14.3, 10.6, -.04, 0, Math.PI * 2); ctx.fill(); ctx.stroke();

    ctx.fillStyle = 'rgba(255,247,237,.88)';
    ctx.beginPath(); ctx.ellipse(3, 6, 7.4, 5.8, -.08, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = 'rgba(255,255,255,.4)';
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.arc(-3,-1,8.5,3.65,5.32); ctx.stroke();

    // Pequeño emblema de estrella cosido al pecho.
    ctx.save();
    ctx.shadowColor = '#fde047'; ctx.shadowBlur = getShadowBlurValue(6,3,0);
    ctx.fillStyle = '#fde047'; drawStarShape(ctx, 4.5, 3.5, 5, 3.4, 1.55);
    ctx.restore();

    // Cabeza redondeada en tres cuartos.
    const headGradient = ctx.createRadialGradient(2, -13, 1, 6, -8, 15);
    headGradient.addColorStop(0, '#ffd08a');
    headGradient.addColorStop(.3, '#fb923c');
    headGradient.addColorStop(.78, '#ea580c');
    headGradient.addColorStop(1, '#9a3412');
    ...
    ctx.beginPath();
    ctx.moveTo(-2,-14); ctx.lineTo(0,-25); ctx.quadraticCurveTo(6,-20,7,-14); ctx.closePath();
    ctx.moveTo(9,-15); ctx.lineTo(16,-24); ctx.quadraticCurveTo(19,-16,17,-11); ctx.closePath();
    ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#fda4af';
    ctx.beginPath();
    ctx.moveTo(.2,-17);ctx.lineTo(1,-22);ctx.lineTo(4.5,-17);ctx.closePath();
    ctx.moveTo(12,-17);ctx.lineTo(15.6,-21.5);ctx.lineTo(16,-15);ctx.closePath();ctx.fill();

    ctx.fillStyle = headGradient;
    ctx.beginPath(); ctx.ellipse(7, -8, 12.7, 11.4, .06, 0, Math.PI * 2); ctx.fill(); ctx.stroke();

    // Marcas naranjas propias de Super Miau.
    ctx.strokeStyle = '#c2410c';
    ctx.lineWidth = 1.5;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(4,-18);ctx.lineTo(5,-14);
    ctx.moveTo(8,-18.5);ctx.lineTo(8,-14.5);
    ctx.moveTo(12,-17.5);ctx.lineTo(11,-14);
    ctx.stroke();

    const blink = tick % 190 > 181;
    if (blink) {
        ctx.strokeStyle = '#3f1d2e'; ctx.lineWidth = 1.7;
        ctx.beginPath();ctx.moveTo(2,-9);ctx.quadraticCurveTo(4,-7.8,6,-9);ctx.moveTo(9,-9);ctx.quadraticCurveTo(11,-7.8,13,-9);ctx.stroke();
    } else {
        ctx.fillStyle = '#fffaf0';
        ctx.beginPath(); ctx.ellipse(4,-9,3.1,3.6,-.08,0,Math.PI*2); ctx.ellipse(11,-9,3.1,3.6,.08,0,Math.PI*2); ctx.fill();
        const look = Math.max(-.7, Math.min(.7, player.vx * .12));
        ctx.fillStyle = '#166534';
        ctx.beginPath(); ctx.ellipse(4.5+look,-8.8,1.7,2.55,0,0,Math.PI*2); ctx.ellipse(11.5+look,-8.8,1.7,2.55,0,0,Math.PI*2); ctx.fill();
        ctx.fillStyle = '#111827';
        ctx.beginPath();ctx.ellipse(4.7+look,-8.7,.72,1.7,0,0,Math.PI*2);ctx.ellipse(11.7+look,-8.7,.72,1.7,0,0,Math.PI*2);ctx.fill();
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();ctx.arc(3.7+look,-10,.65,0,Math.PI*2);ctx.arc(10.7+look,-10,.65,0,Math.PI*2);ctx.fill();
    }

    // Hocico, nariz, sonrisa y bigotes.
    ctx.fillStyle = '#fff1dc';
    ctx.beginPath();ctx.ellipse(5.2,-4.1,4.2,3.1,-.08,0,Math.PI*2);ctx.ellipse(10,-4.1,4.2,3.1,.08,0,Math.PI*2);ctx.fill();
    ctx.fillStyle = '#f43f5e';
    ctx.beginPath();ctx.moveTo(7.6,-5.1);ctx.quadraticCurveTo(10,-5.2,9.2,-3.3);ctx.quadraticCurveTo(7.6,-2.4,6.2,-3.5);ctx.closePath();ctx.fill();
    ctx.strokeStyle = '#7c2d12';ctx.lineWidth = 1;ctx.beginPath();ctx.moveTo(7.7,-3.2);ctx.quadraticCurveTo(7.4,-1.4,5.6,-1.2);ctx.moveTo(7.7,-3.2);ctx.quadraticCurveTo(8.2,-1.4,10,-1.2);ctx.stroke();
    ctx.strokeStyle = 'rgba(92,45,24,.7)';ctx.lineWidth = .75;
    ctx.beginPath();ctx.moveTo(3,-4);ctx.lineTo(-4,-5.5);ctx.moveTo(3,-2.5);ctx.lineTo(-4,-1.5);ctx.moveTo(12,-4);ctx.lineTo(18,-5.2);ctx.moveTo(12,-2.5);ctx.lineTo(18,-1);ctx.stroke();

    // Broche dorado que fija la capa al cuello.
    ctx.fillStyle = '#7f1d1d';
    ctx.beginPath();ctx.ellipse(-2,-6.5,4.5,2.2,-.3,0,Math.PI*2);ctx.fill();
    ctx.shadowColor = '#fde047';ctx.shadowBlur = getShadowBlurValue(5,2,0);
    ctx.fillStyle = '#fde047';ctx.beginPath();ctx.arc(-3.5,-7,1.8,0,Math.PI*2);ctx.fill();
...
// Bucle Principal
let lastTime = 0;
// Es el corazon del juego: organiza actualizacion, render y control del tiempo entre frames.
